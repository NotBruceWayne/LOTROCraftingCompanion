...
-- Repair and complete the Turbine.Gameplay.CraftTier enumeration
...
-- This function gets the localized craft tier name from the value (e.g. 7 == Westfold).
function GetCraftTier(TIERVALUE)
	for k, v in pairs (Turbine.Gameplay.CraftTier) do
		if v == TIERVALUE then
            return _LANG.CHATLOG.TIERS[k][SETTINGS.LANGUAGE];
		end
	end
end
